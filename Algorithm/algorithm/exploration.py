...
class Exploration:
	...
	def run(self):
		if self._robot.is_actual_robot():
			...
			while True:
				...
				if _spwp[:2] == 'SP':
					...
					self._robot.notify_arduino(Action.LEFT)
					CommMgr.recv()
					self._robot.notify_arduino(Action.CALIBRATE)
					CommMgr.recv()
					self._robot.notify_arduino(Action.LEFT)
					CommMgr.recv()
					self._robot.notify_arduino(Action.CALIBRATE)
					CommMgr.recv()
					self._robot.notify_arduino(Action.RIGHT)
					CommMgr.recv()
					self._robot.notify_arduino(Action.RIGHT)
					CommMgr.recv()
		...
		if self._robot.is_actual_robot():
			...
			# self._robot.notify_arduino(Action.RIGHT)
			# CommMgr.recv()
			self._robot.notify_arduino(Action.SENSOR)#, self._explored_map, False)
	...
	def do_exploration(self, pos):
		...
		if self._wp is not None:
			...
			_status = _go_to_wp_goal.do_fastest_path(self._wp)
	...
	def go_home(self):
		...
		self.turn_robot(Orientation.NORTH.value, True)
	...
	def calibrate(self, target_ori):
		_cur_ori = self._robot.get_ori()
		self.turn_robot(target_ori, False)
		self.move_robot(Action.CALIBRATE)#, self._explored_map, False)
		self.turn_robot(_cur_ori, False)
	...
	def turn_robot(self, _ori_target, to_android):
